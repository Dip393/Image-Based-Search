...
def load_classification_model():
    ...
    model.load_state_dict(torch.load(os.path.join(os.getcwd(), "model_weights_edir", "plant_classifier.pth")))
    model.eval()
    return model
...
def load_similarity_data():
    all_vecs = np.load(os.path.join(os.getcwd(), "features", "all_vecs.npy"))
    all_names = np.load(os.path.join(os.getcwd(), "features", "all_names.npy"))
...
def load_plant_info(plant_name):
    ...
    try:
        with open(os.path.join(os.getcwd(), "plant_info", f"{plant_name}.txt"), "r", encoding='utf-8') as file:
            return file.read()
...
def extract_features(img, model, transform):
    ...
    with torch.no_grad():
        features = model(img_tensor)  # Get raw output from the model (before softmax)
    return features.squeeze(0).cpu().numpy()  # Remove batch dimension and convert to numpy array

# Function to calculate cosine similarity
def calculate_similarity(feature1, feature2):
    return cdist([feature1], [feature2], metric='cosine')[0][0]
...
if uploaded_file:
    ...
    # Compare the uploaded image's features to the precomputed features of other plants
    uploaded_features = extract_features(img, classification_model, transform)
    similarities = []
    
    for i, vec in enumerate(vecs):
        similarity = calculate_similarity(uploaded_features, vec)
        similarities.append((similarity, names[i]))

    # Sort the images by similarity (lower similarity = more similar)
    similarities.sort(key=lambda x: x[0])

    # Display the most similar images (top 5)
    cols = st.columns(5)
    for i, col in enumerate(cols):
        if i < len(similarities):
            similar_plant_name = similarities[i][1]
            try:
                # Fetch the image from the URL (already stored in names)
                response = requests.get(similar_plant_name)
                img_similar = Image.open(BytesIO(response.content))

                col.image(img_similar)
                col.write(f"Similarity Score: **{(1 - similarities[i][0]) * 100:.2f}%**")
